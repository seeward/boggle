...
var currentWord = {};
// load the dictionary
fetch('https://seeward.github.io/CODEPLANT_v1/public/media/words.json').then((results) => {
...
var isAdjacent = (prevLetter, currLetter) => {
  // get both indexs
...
var isAdjacentDiceFromIndex = (currentIndex, prevIndex) => {
  console.log(workCheckMap[prevIndex + 1], prevIndex);
  var check = false;
  if (workCheckMap[prevIndex + 1]) {
    workCheckMap[prevIndex + 1].forEach((eachAdjacentTile) => {
...
var walkWord = (word) => {
  var wordAsArray = [...word];
  return recursiveLoop(wordAsArray)
}

var recursiveLoop = (wordArray) => {
